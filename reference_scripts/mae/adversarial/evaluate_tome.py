...
def _load_client_tome(vit, K):
    ckpt = torch.load(_client_root / f"s{K}{FILE_TAG}.pth", map_location=DEVICE)
    client = ClientVitToMe(vit, K, enable_pos_embed=ENABLE_PE)
...
def run():
    ...
    for K in args.split:
        print(f"\n{'#'*60}\n  Adversarial — ToMe  Split={K}  mode={FILE_TAG}  [{MODEL_NAME}]\n{'#'*60}")

        vit_c  = _load_vit()
        vit_s  = _load_vit()
        client = _load_client_tome(vit_c, K)
        ...
        for r in args.r_values:
            ...
            label = f"Adversarial ToMe r={r} (~{approx_tokens} tokens) [SARA] [{MODEL_NAME}] split={K} mode={FILE_TAG}"
            print(f"\n{'='*60}\n  {label}\n{'='*60}")
