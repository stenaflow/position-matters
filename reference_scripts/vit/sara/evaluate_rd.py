...
def run():
    ...
    for K in args.split:
        print(f"\n{'#'*60}")
        print(f"  Split block = {K}")
        print(f"{'#'*60}")

        vit    = timm.create_model("vit_base_patch16_224", pretrained=True).to(DEVICE).eval()
        client = ClientVitRD(vit=vit, split_block_number=K).to(DEVICE).eval()
        ...
        for r in args.r_values:
            ...
            label = f"Random Dropping r={r} (~{approx_tokens} tokens) [SARA] split={K}"
            print(f"\n{'='*60}\n  {label}\n{'='*60}")
            ...
            result = tracker.result(label, r=r, approx_tokens=approx_tokens,
                                    split_block=K)
