...
def _load_client_rd(K):
    ckpt_path = _client_root / f"s{K}{FILE_TAG}.pth"
    ckpt = torch.load(ckpt_path, map_location=DEVICE)
    vit  = timm.create_model("vit_base_patch16_224", pretrained=True)
    client = ClientVitRD(vit, K, enable_pos_embed=ENABLE_PE)
...
def run():
    ...
    results_dir = _script_dir / "results" / f"rd{ct_suffix}"
    results_dir.mkdir(parents=True, exist_ok=True)

    for K in args.split:
        print(f"\n{'#'*60}\n  Adversarial — RD  Split={K}  mode={FILE_TAG}\n{'#'*60}")

        vit    = timm.create_model("vit_base_patch16_224", pretrained=True).to(DEVICE).eval()
        client = _load_client_rd(K)
        ...
        for r in args.r_values:
            ...
            label = f"Adversarial RD r={r} (~{approx_tokens} tokens) [SARA] split={K} mode={FILE_TAG}"
            print(f"\n{'='*60}\n  {label}\n{'='*60}")

            tracker.reset()
            for x, y in tqdm(loader, desc=f"s{K}{FILE_TAG} r={r}"):
            ...
            existing[r] = result
            out_path.write_text(json.dumps(sorted(existing.values(), key=lambda x: x["r"]), indent=4))
