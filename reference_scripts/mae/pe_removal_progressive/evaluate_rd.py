...
def _load_client_rd(vit, K):
    ...
    client = ClientVitRD(vit, K, enable_pos_embed=False)
...
def run():
    ...
    for K in args.split:
        print(f"\n{'#'*60}\n  PE-Removal Progressive RD — Split={K}  [{MODEL_NAME}]\n{'#'*60}")
        ...
        for r in args.r_values:
            ...
            label = f"PE-Removal Progressive RD r={r} (~{approx_tokens} tokens) [SARA] [{MODEL_NAME}] split={K}"
            print(f"\n{'='*60}\n  {label}\n{'='*60}")
