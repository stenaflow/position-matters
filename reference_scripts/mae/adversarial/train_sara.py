...
_mode.add_argument("--adv-only",        action="store_true", dest="adv_only",
                   help="evaluate adversarial-only client (PE kept, _adv_only suffix)")
_mode.add_argument("--pe-removal-only", action="store_true", dest="pe_removal_only",
                   help="evaluate PE-removal-only client (_pe_only suffix)")
...
def load_client(K):
    ckpt_path = _client_root / f"s{K}{FILE_TAG}.pth"
    if not ckpt_path.exists():
        flag = " --adv-only" if args.adv_only else (" --pe-removal-only" if args.pe_removal_only else "")
        raise FileNotFoundError(f"Adversarial client not found: {ckpt_path}\n"
                                f"Run train_client.py --split {K}{flag} first.")
...
def _train_tpp(client, train_dl, val_dl, save_path):
    print(f"\n{'='*60}\n[1/3] ContextualPositionPredictor (fresh -- NOT the adversarially "
          f"co-trained one)\n{'='*60}")
...
def main():
    ...
    if args.pe_removal_only:
        mode = "PE-removal only"
    elif args.adv_only:
        mode = "adversarial only (PE intact)"
    else:
        mode = "adversarial + PE removal (full)"

    for K in args.split:
        print(f"\n{'#'*60}\n  Adversarial SARA (fresh attacker) [{MODEL_NAME}]  split={K}  mode={mode}\n{'#'*60}")

        client = load_client(K)
        for p in client.parameters():
            p.requires_grad_(False)

        tpp_path = _sara_root / "tpp"     / f"s{K}{FILE_TAG}.pth"
        mae_path = _sara_root / "mae"     / f"s{K}{FILE_TAG}.pth"
        dec_path = _sara_root / "decoder" / f"s{K}{FILE_TAG}.pth"
