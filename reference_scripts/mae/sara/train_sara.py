...
def _train_tpp(client, train_dl, val_dl, save_path):
    print(f"\n{'='*60}\n[1/2] ContextualPositionPredictor\n{'='*60}")
...
def main():
    ...
    for K in args.split:
        print(f"\n{'#'*60}\n  Training SARA  split={K}  [{MODEL_NAME}]\n{'#'*60}")
        ...
        tpp_path = MODELS_ROOT / "sara" / "tpp" / f"s{K}.pth"
        mae_path = MODELS_ROOT / "sara" / "mae" / f"s{K}.pth"
        tpp_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        if args.mae_only:
            ...
            tpp.load_state_dict(_ckpt.get("state_dict", _ckpt)); tpp.eval()
            print(f"  [skip] Loaded existing TPP")
        else:
            tpp = _train_tpp(client, train_dl, val_dl, tpp_path)

        _train_mae(client, train_dl, val_dl, K, mae_path)
