...
parser.add_argument("--pe-only-epochs",   type=int,   default=15,  dest="pe_only_epochs",
                    help="epochs per block for --only-pe-removal mode")
parser.add_argument("--pe-only-patience", type=int,   default=8,   dest="pe_only_patience",
                    help="early-stop patience for --only-pe-removal mode")
# ── shared ──
...
parser.add_argument("--norm-reg",       type=float, default=0.0,   dest="norm_reg",
                    help="L2-norm regularization weight on patch tokens to prevent magnitude blow-up "
                         "(add μ·mean(||z_patches||²) to the client loss; 0 = disabled)")
parser.add_argument("--resume-from",    type=str,   default=None,  dest="resume_from",
                    help="path to a client checkpoint (.pth) to merge into the ViT before training, "
                         "overriding the automatic s{min(split)-1}{FILE_TAG}.pth resume "
                         "(e.g. saved_models/adversarial/client/s8_full.pth to resume PE-only from an "
                         "Adversarial-trained base)")
...
if args.pe_removal_only:
    ...
    FILE_TAG    = "_adv_only"
    LOG_SUBDIR  = "adv_only"
else:  # default: adversarial + PE removal
    ENABLE_PE   = False
    FILE_TAG    = "_full"
    LOG_SUBDIR  = "full"
...
def _make_tpp_batch(smashed):
    ...
    return F.normalize(inp, dim=-1), gt_pos[bidx, perm][:, 1:]
...
def _freeze_up_to(vit, K):
    vit.patch_embed.requires_grad_(False)
    vit.cls_token.requires_grad_(False)
    for i in range(K):
        vit.blocks[i].requires_grad_(False)
...
def _train_round(name, vit, vit_teacher, tpp,
    ...
    for ep in range(1, args.epochs + 1):
        ...
        t_util = t_correct = t_total = t_tpp_loss = t_tpp_correct = t_tpp_total = t_norm = n_batches = 0
        for x, y in tqdm(train_dl, desc=f"  [{name}] ep {ep}/{args.epochs} [train] lam={lam:.3f}"):
            ...
            with torch.amp.autocast("cuda", enabled=AMP):
                ...
                loss_norm   = z[:, 1:].norm(dim=-1).pow(2).mean() if args.norm_reg > 0.0 else 0.0
                loss_client = loss_util - lam * loss_pos_capped + args.norm_reg * loss_norm
            ...
            t_util += loss_util.item()
            t_norm += loss_norm.item() if args.norm_reg > 0.0 else 0.0
            t_total += y.size(0); n_batches += 1
        sched_c.step()
        ...
        norm_str = f" │ norm={t_norm/n_batches:.2f}" if args.norm_reg > 0.0 else ""
        print(f"  [{name}] ep {ep:>3} │ lam={lam:.3f} │ "
              f"util={val_util:.4f} acc={val_acc*100:5.2f}% │ "
              f"tpp loss={val_tpp_loss:.4f} acc={val_tpp_acc*100:5.2f}%{norm_str}")
        ...
        log.append({
            ...
            "train_norm_loss": round(t_norm / n_batches, 6) if args.norm_reg > 0.0 else 0.0,
...
def main():
    ...
    resume_from = min(args.split) - 1
    if args.resume_from is not None:
        resume_path = Path(args.resume_from)
        if not resume_path.exists():
            raise FileNotFoundError(f"--resume-from checkpoint not found: {resume_path}")
        print(f"\nResuming from {resume_path} (explicit --resume-from)"
              f" -- loading blocks 1-{resume_from} (frozen)")
        resume_ckpt = torch.load(resume_path, map_location=DEVICE)
        full_sd = vit.state_dict()
        full_sd.update(resume_ckpt["client_state_dict"])
        vit.load_state_dict(full_sd)
        if resume_from >= 1:
            _freeze_up_to(vit, resume_from)
    elif resume_from >= 1:
        resume_path = client_dir / f"s{resume_from}{FILE_TAG}.pth"
        if not resume_path.exists():
            raise FileNotFoundError(
                f"--split starts at {min(args.split)}, but no checkpoint found for the "
                f"preceding block: {resume_path}\n"
                f"Run train_client.py for split {resume_from} first (or include it in "
                f"--split), or pass --resume-from explicitly if it was trained in a "
                f"different mode, so blocks 1-{resume_from} can be loaded before "
                f"training block {min(args.split)}.")
        print(f"\nResuming from {resume_path} -- loading blocks 1-{resume_from} (frozen)")
        resume_ckpt = torch.load(resume_path, map_location=DEVICE)
        full_sd = vit.state_dict()
        full_sd.update(resume_ckpt["client_state_dict"])
        vit.load_state_dict(full_sd)
    ...
    for K in sorted(args.split):
        log_dir = _script_dir / "training" / f"s{K}" / LOG_SUBDIR
        log_dir.mkdir(parents=True, exist_ok=True)

        trainable_params, trainable_modules = _get_trainable(vit, K)

        if args.pe_removal_only:
            print(f"\n{'#'*60}\n  Adversarial  split={K}  ({mode})"
