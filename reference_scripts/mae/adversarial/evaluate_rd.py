...
def run():
    ...
    results_dir = _here / "results" / f"rd{ct_suffix}"
    results_dir.mkdir(parents=True, exist_ok=True)

    for K in args.split:
        print(f"\n{'#'*60}\n  Adversarial — RD  Split={K}  mode={FILE_TAG}  [{MODEL_NAME}]\n{'#'*60}")

        vit    = _load_vit()
        client = _load_client_rd(vit, K)
        ...
        for r in args.r_values:
            ...
            label = f"Adversarial RD r={r} (~{approx_tokens} tokens) [SARA] [{MODEL_NAME}] split={K} mode={FILE_TAG}"
            print(f"\n{'='*60}\n  {label}\n{'='*60}")
