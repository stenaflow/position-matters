...
def _load_client(K):
    ...
    return client.to(DEVICE).eval(), vit
...
def run():
    ...
    for K in args.split:
        print(f"\n{'#'*60}\n  Adversarial — Shuffling  Split={K}  mode={FILE_TAG}\n{'#'*60}")

        client, vit = _load_client(K)
        ...
        if out_path.exists():
            ...
            label  = f"Adversarial Shuffling [SARA] split={K} mode={FILE_TAG}"
